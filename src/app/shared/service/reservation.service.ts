...
//         environment.Api+'check2',this.SendData,{headers} ).subscribe(
//           (ResData) => {
//            console.log(ResData)
...
//     environment.Api+'get-reservation',{headers})
//   }
  ...
  console.log(data);  
    const headers = new HttpHeaders({
     ...
     this.Http.post(environment.Api+`payment`,data,{headers}).subscribe(
       ...
       console.log(next)
       this.Http.post<Reservation>(
        environment.Api+'reserve',this.SendData,{headers} ).subscribe(
          (ResData) => {
           console.log(ResData)
           this.OrderService.DeleteOrder()
           this.checked.next(true)
